...
export default function TIKI2() {
  ...
  useEffect(() => {
    ...
    if (typeof window !== 'undefined') {
      try {
        const draft = localStorage.getItem('test_draft_answers_tiki2');
        if (draft) {
  ...
  const handleFinish = async () => {
    ...
    if (typeof window !== 'undefined') {
      localStorage.removeItem('test_draft_answers_tiki2');
      localStorage.removeItem('test_timer_left_tiki2');
  ...
  const handleAnswer = (val: string) => {
    ...
    setAnswers(updated);
    if (typeof window !== 'undefined') {
      localStorage.setItem('test_draft_answers_tiki2', JSON.stringify(updated));
    }
  };
