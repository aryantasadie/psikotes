...
export default function IST2() {
  ...
  useEffect(() => {
    ...
    if (typeof window !== 'undefined') {
      try {
        const draft = localStorage.getItem('test_draft_answers_ist2');
        if (draft) {
  ...
  const handleFinish = async () => {
    ...
    if (typeof window !== 'undefined') {
      localStorage.removeItem('test_draft_answers_ist2');
      localStorage.removeItem('test_timer_left_ist2');
  ...
  const handleAnswer = (val: string) => {
    ...
    if (qItem) {
      ...
      setAnswers(updated);
      if (typeof window !== 'undefined') {
        localStorage.setItem('test_draft_answers_ist2', JSON.stringify(updated));
      }
    }
  };
