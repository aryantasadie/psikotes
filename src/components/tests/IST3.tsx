...
export default function IST3() {
  ...
  useEffect(() => {
    ...
    if (typeof window !== 'undefined') {
      try {
        const draft = localStorage.getItem('test_draft_answers_ist3');
        if (draft) {
  ...
  const handleFinish = async () => {
    ...
    if (typeof window !== 'undefined') {
      localStorage.removeItem('test_draft_answers_ist3');
      localStorage.removeItem('test_timer_left_ist3');
  ...
  const handleAnswer = (val: string) => {
    ...
    if (qItem) {
      ...
      setAnswers(updated);
      if (typeof window !== 'undefined') {
        localStorage.setItem('test_draft_answers_ist3', JSON.stringify(updated));
      }
    }
  };
