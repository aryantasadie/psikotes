    ...
    return (
      <div style={{ padding: '30px', fontFamily: '"Inter", sans-serif', background: '#f4f7f6', minHeight: '100vh', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
        <div style={{ maxWidth: '700px', background: 'white', padding: '50px', borderRadius: '24px', boxShadow: '0 20px 40px rgba(0,0,0,0.08)', textAlign: 'center' }}>
          <h2 style={{ fontSize: '32px', color: '#2c3e50', marginBottom: '20px', fontWeight: '800' }}>IST 3 - Analogi Kata</h2>
          <div style={{ textAlign: 'left', background: '#f8fbff', padding: '25px', borderRadius: '12px', borderLeft: '6px solid #3498db', marginBottom: '35px', lineHeight: '1.7', color: '#444', fontSize: '16px' }}>
            ...
            <div style={{ background: '#fff', padding: '20px', borderRadius: '12px', border: '1px solid #ddd', marginTop: '20px' }}>
              ...
              <div style={{ background: '#f9f9f9', padding: '15px', borderRadius: '8px', marginBottom: '10px' }}>
                ...
                <p style={{ margin: '0 0 5px 0', fontWeight: 'bold' }}>a) Batu bata &nbsp; b) Rumah &nbsp; c) Semen &nbsp; d) Putih &nbsp; e) Dinding</p>
              </div>
              <p style={{ margin: '0', fontSize: '15px', color: '#e67e22', fontWeight: 'bold' }}>Jawaban: a (Batu bata)</p>
              <p style={{ margin: '5px 0 0 0', fontSize: '14px', color: '#555' }}>Penjelasan: Hutan terdiri atas pohon-pohon, maka tembok terdiri atas batu bata.</p>
            </div>
  ...
  return (
    <div style={{ padding: '30px', fontFamily: '"Inter", sans-serif', background: '#f4f7f6', minHeight: '100vh', color: '#333', display: 'flex', alignItems: 'center' }}>
      <div style={{ maxWidth: '1100px', margin: '0 auto', display: 'flex', gap: '30px', alignItems: 'flex-start', flexWrap: 'wrap', width: '100%' }}>
        ...
        <div style={{ marginBottom: '40px', minHeight: '100px', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', width: '100%' }}>
          {q.content && q.content.includes('=') ? (
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '20px', fontSize: '20px', fontWeight: 'bold', flexWrap: 'wrap', width: '100%' }}>
              <div style={{ flex: '1', background: '#f8fbff', padding: '20px 25px', borderRadius: '12px', border: '2px solid #e1ebf5', color: '#2c3e50', textAlign: 'center', letterSpacing: '1px', boxShadow: '0 4px 6px rgba(0,0,0,0.02)' }}>
                {q.content.split('=')[0].replace(/\s+/g, ' ').trim()}
              </div>
              <div style={{ color: '#3498db', fontSize: '36px', fontWeight: '900' }}>=</div>
              <div style={{ flex: '1', background: '#f8fbff', padding: '20px 25px', borderRadius: '12px', border: '2px solid #e1ebf5', color: '#2c3e50', textAlign: 'center', letterSpacing: '1px', boxShadow: '0 4px 6px rgba(0,0,0,0.02)' }}>
                {q.content.split('=')[1].replace(/\s+/g, ' ').trim()}
              </div>
            </div>
          ) : (
