...
export default function IST7() {
  ...
  useEffect(() => {
    ...
    if (typeof window !== 'undefined') {
      try {
        const draft = localStorage.getItem('test_draft_answers_ist7');
        if (draft) {
  ...
  const handleFinish = async () => {
    ...
    if (typeof window !== 'undefined') {
      localStorage.removeItem('test_draft_answers_ist7');
      localStorage.removeItem('test_timer_left_ist7');
  ...
  const handleAnswer = (val: string) => {
    ...
    if (qItem) {
      ...
      setAnswers(updated);
      if (typeof window !== 'undefined') {
        localStorage.setItem('test_draft_answers_ist7', JSON.stringify(updated));
      }
    }
  };
