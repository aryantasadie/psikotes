...
export default function CFIT2() {
  ...
  useEffect(() => {
    if (typeof window !== 'undefined') {
      try {
        const draft = localStorage.getItem('test_draft_answers_cfit2');
        if (draft) {
  ...
  const handleFinish = async () => {
    ...
    if (typeof window !== 'undefined') {
      localStorage.removeItem('test_draft_answers_cfit2');
      localStorage.removeItem('test_timer_left_cfit2');
  ...
  const handleAnswer = (val: string) => {
    ...
    if (typeof window !== 'undefined') {
      localStorage.setItem('test_draft_answers_cfit2', JSON.stringify(newAnswers));
    }
  };
