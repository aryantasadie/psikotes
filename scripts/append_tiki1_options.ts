...
async function main() {
  ...
  for (let i = 0; i < 40; i++) {
    ...
    let opts = qData.options.map((opt: string) => {
       return opt.replace(/\u2044/g, '/').replace(/\u00a0/g, ' ').replace(/\u00bd/g, '1/2');
    });

    let appendedContent = `${formattedQuestions[i]}\n\n${opts[0]}      ${opts[1]}\n${opts[2]}      ${opts[3]}`;
  ...
  console.log("Successfully appended options to TIKI 1 content.");
}
